...
function arrayStore(filePath){return createAtomicJsonStore({filePath,initialValue:[],validate:v=>({ok:Array.isArray(v),errors:['array_required']})});}
export function createCaseStore({filePath}){const s=arrayStore(filePath);return{
...
export function createPartnerStore({filePath}){const s=createAtomicJsonStore({filePath,initialValue:{organizations:[],users:[],availability:[]},validate:v=>({ok:v&&Array.isArray(v.organizations)&&Array.isArray(v.users)&&Array.isArray(v.availability),errors:['partner_store_invalid']})});return{
...
export function createSessionStore({filePath}){const s=createAtomicJsonStore({filePath,initialValue:{magicLinks:[],sessions:[]},validate:v=>({ok:v&&Array.isArray(v.magicLinks)&&Array.isArray(v.sessions),errors:['session_store_invalid']})});return{
